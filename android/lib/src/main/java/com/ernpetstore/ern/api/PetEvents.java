...
final class PetEvents implements PetApi.Events {
    ...
    @Override
    public UUID addImageUploadedEventListener(@NonNull final ElectrodeBridgeEventListener<Long> eventListener) {
        return new EventListenerProcessor<>(EVENT_IMAGE_UPLOADED, Long.class, eventListener).execute();
    }

                @Override
                public ElectrodeBridgeEventListener<ElectrodeBridgeEvent> removeImageUploadedEventListener(@NonNull final UUID uuid) {
